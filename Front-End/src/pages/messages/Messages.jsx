...
import {
  ...
  fetchMessages,
  pushMessage,
} from "../utils/socket"; // Import connectSocket and joinChannel
...
const Messages = ({ userId, users }) => {
  ...
  useEffect(() => {
    ...
    const setupChannel = async () => {
      try {
        ...
        const { channel } = await joinChannel("room:lobby", {
          user_id: userId,
        });
        ...
        const existingMessages = await fetchMessages("room:lobby", {
          user_id: userId,
        });
  ...
  return (
    <div className="messages">
      {!connected && (
        <div className="connection-status">Attempting to connect...</div>
      )}
      {/* Message list */}
